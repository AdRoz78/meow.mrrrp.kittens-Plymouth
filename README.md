...
## Installation
Download the release and extract the `meow.mrrp.kittens` folder to `/usr/share/plymouth/themes`. Afterwards run `sudo plymouth-set-default-theme meow.mrrp.kittens`.

## Notes
This was made for a 2K (2560x1440) display, on other screen sizes the progress bar will be wrong. To fix this open the `meow.mrrp.kittens.plymouth` file inside the directory and edit the `ProgressBarWidth=2560` line, replacing the number with your screen width.
